...
(
    ...
    `is_default_address`  TINYINT         NULL        COMMENT '기본 배송지 여부', 
...
(
    ...
    `is_answered`       TINYINT     NULL        Default False    COMMENT '처리상태(답변여부)', 
    `is_secreted`       TINYINT     NOT NULL    Default False    COMMENT '공개여부', 
    PRIMARY KEY (id)
);
...
(
    `id`                      INT             NOT NULL    AUTO_INCREMENT, 
    `order_number`            VARCHAR(128)    NOT NULL    COMMENT '202009290000000017', 
    `final_price`             INT             NOT NULL    COMMENT '최종 결제 금액', 
    `order_date`              DATETIME        NOT NULL    COMMENT '주문일시', 
    `user_id`                 INT             NOT NULL    COMMENT '구매자(유저)아이디', 
    `shipping_infomation_id`  INT             NOT NULL    COMMENT '배송지 정보 아이디', 
